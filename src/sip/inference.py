...
def _active_art():
    """Primary artifact for shared helpers (BGE model name, encoder prefix). The deployed product is
    the unified classifier, so prefer unified.joblib; fall back to the legacy deployable.joblib only
    if unified is absent. Keeps the runtime dependent on a SINGLE model file."""
    try:
        return _load_unified()
    except Exception:
        return _load()
...
# ---------------------------------------------------------------- feature vector
def _hand_blockvals(inp: dict) -> tuple[dict, dict, str]:
    """Deterministic hand features (caption/timing/duration/meta) from raw input.
    Returns (blockvals by feature name, caption-feature dict, caption string)."""
    ...
    return {**cap, **timing, **duration, **meta}, cap, caption
...
def _feature_vector(inp: dict) -> tuple[np.ndarray, dict]:
    ...
    art = _load()
    blockvals, cap, caption = _hand_blockvals(inp)
    # text embedding (live)
...
def _load_unified():
    global _ARTIFACT_U
    if _ARTIFACT_U is None:
        p = C.MODELS / "unified.joblib"
        if not p.exists():
            raise FileNotFoundError("models/unified.joblib missing — run experiments/build_unified.py")
        _ARTIFACT_U = joblib.load(p)
    return _ARTIFACT_U


def predict_unified(inp: dict, mm_feats: dict | None = None, day_one: dict | None = None,
                    cost_ratio: float | None = None) -> dict:
    """THE one deployed classifier: hand features + BGE(caption+transcript) + SigLIP + CLAP,
    trained on ~12.7k real TikTok (creator-split). Day-1 is strictly opt-in: the day-1 head is
    used ONLY when `day_one` is passed; otherwise the no-day-1 head — day-1 never leaks in.
    `mm_feats` (from sip.extract.mm_features) may be None -> zeros + has_video/has_audio=0,
    matching the training encoding of a missing modality."""
    art = _load_unified()
    m = art["Ud1"] if day_one else art["U"]
    blockvals, cap, caption = _hand_blockvals(inp)
    extra = str(inp.get("extra_text") or "")
    bge = _bge((caption + " " + extra).strip())
    mm = mm_feats or {}
    has_audio = float(any(k.startswith("clap") and _safe_float(v, 0.0) != 0.0 for k, v in mm.items()))
    has_video = float(any(k.startswith("vsig") for k in mm))
    flags = {"has_caption": float(bool(caption.strip())),
             "has_transcript": float(bool(extra.strip())),
    ...
    if day_one:
        ...
        d1 = {"log_play_d1": np.log1p(play), "log_like_d1": np.log1p(g("like_count")),
              "er_d1": eng / max(play, 1.0)}
    enc = art.get("encoder_name", "bge")
    vec = []
    for name in m["names"]:
        if name in blockvals:
            vec.append(blockvals[name])
        elif name in flags:
            vec.append(flags[name])
        elif name in d1:
            vec.append(d1[name])
        elif name.startswith(("vsig", "clap")):
            vec.append(_safe_float(mm.get(name), 0.0))     # zero-imputed like training
    ...
    bands = _bands_for_cost(m, cost_ratio)
    dec = _decide(p, bands, m["conformal"])
    factors = _shap_factors(x, m) or _factors(cap, inp)
    warnings = _validate(inp)
    if mm_feats is None:
        warnings.append("No video features available for this input — scored from text+metadata only "
                        "(has_video=0), the same encoding the model saw in training.")
    return {
        "recommendation": dec, "probability": round(p, 4),
        "model": ("unified + day-1" if day_one else "unified (text + video + audio)"),
        "used_day1": bool(day_one),
        "bands": bands, "cost_ratio": cost_ratio, "factors": factors,
        "examples": _examples_from(art, bge),
        "what_model_cannot_know": _CANNOT_KNOW,
        "label_meaning": art["label_definition"],
        "calibrated": True, "warnings": warnings,
        "rationale": _rationale(dec, p, factors, "B" if day_one else "A"),
        "metrics_note": (f"test ROC-AUC {m['metrics']['roc_auc']} on held-out creators, "
                         f"n={art['trained_on']['n']} real TikTok"),
    }


def _examples_from(art, bge, k=3):
    """Nearest past videos from a given artifact's example bank."""
    bank = art.get("example_bank")
...
def _shap_factors(x, m, topn=6):
    ...
    try:
        ...
        names = m["names"]; enc = _active_art().get("encoder_name", "bge")
        agg = {}
